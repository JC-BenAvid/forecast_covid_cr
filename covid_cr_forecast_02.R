### Corona virus CR Forecast based on time series and stochastic processes, 21th May 21
...
#loading data
ruta<-#"local path file location\\covid_cr_data_act21may.csv"
...
#exploring full data
length(covid_cr)
plot(covid_cr, xlab= "Index Days", ylab = "Cases", main= "Covid Cases CR - 21th May 2021")
acf(covid_cr, lag.max = 50, main='ACF of Original data Covid CR')
pacf(covid_cr, lag.max = 50,  main='PACF of Original data Covid CR')

#Looking for the best transformation, keeping whole data (try and error)
t1<-log(covid_cr)
plot(t1) #is not WN
...
#Looking for the best transformation, cutting data (try and error)
plot.ts(covid_cr[100:440], xlab= "Index Days", ylab = "Cases", main= "Covid Cases CR (100 days cut) - 21th May 2021")
zoom_covid_cr<-ts(covid_cr[100:440])
...
plot(t, xlab= "Index Days", ylab = "Transformation Data", main= "White Noise transformation data - 21th May 2021") #this is the most similar to WN
...
par(mfrow=c(1,1))
plot(forecast(model),xlab= "Index Days", ylab = "Transformed Cases", main= "Forecast Covid Cases CR - 21th May")
forecast(model,12)
