### Corona virus CR Forecast based on time series and stochastic processes, 9th Jun 21
...
#loading data
ruta<-#"local path file location\\covid_cr_data_act_10junio.csv"
...
#exploring full data
length(covid_cr)
plot(covid_cr, xlab= "Days", ylab = "Cases", main= "Covid Cases CR, May 21th 2021")
...
plot.ts(covid_cr[100:460])
zoom_covid_cr<-ts(covid_cr[100:460])
...
plot(t) #this is the most similar to WN
Box.test(t, lag=log(length(t))) #we get a very very low p-value according to theory
par(mfrow=c(1,1))
acf(zoom_covid_cr, lag.max = 50, main='ACF of Cutted data Covid CR')
pacf(zoom_covid_cr, lag.max = 50,  main='PACF of Cutted data Covid CR')
...
par(mfrow=c(1,1))
plot(forecast(model),xlab= "Days", ylab = "Cases", main= "Forecast Covid Cases CR, June 10th")
forecast(model,12)
